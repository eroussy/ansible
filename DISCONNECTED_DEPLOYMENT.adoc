...
TLS is enabled by default (`podman_registry_mirror_tls_enabled: true`). When no certificate paths are provided, the registry role automatically generates a self-signed CA and server certificate. The CA certificate is then distributed to all cluster nodes so they trust the registry without insecure flags.
...
TODO: This must now use seapath-cluster.yaml example and the seapath setup main playbook
"Disconnected deployment" is only available if the variable create_cephadm_registry is true
