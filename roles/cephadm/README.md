...
| cephadm_release               | No       | String | `ceph_version` or "20.2.0" | Version of the cephadm binary to install                                            |
...
| cephadm_registry_url          | No       | String | `registry_url`      | URL of the container registry. If not set, a local podman registry mirror is used          |
| cephadm_registry_auth_enabled | No       | Bool   | false               | Authenticate to the ceph container image registry using a password                         |
| cephadm_registry_username     | No*      | String | `registry_username` | Username for the container registry. Required if `cephadm_registry_auth_enabled` is true.  |
| cephadm_registry_password     | No*      | String | `registry_passwd`   | Password for the container registry. Required if `cephadm_registry_auth_enabled` is true . |
